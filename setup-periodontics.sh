#!/bin/bash

# Setup Periodontics System
# This script sets up the database and dependencies for the Periodontics module

echo "🦷 Setting up Periodontics System..."

# Navigate to backend
cd minimal-api-dev-v6

echo "📦 Installing dependencies..."
npm install

echo "🗄️  Generating Prisma Client..."
npx prisma generate

echo "📊 Creating database migration..."
npx prisma migrate dev --name add_periodontal_charts

echo "✅ Backend setup complete!"

# Navigate to frontend
cd ../vite-js

echo "📦 Installing frontend dependencies..."
npm install

echo "✅ Frontend setup complete!"

echo ""
echo "🎉 Periodontics System is ready!"
echo ""
echo "To start the servers:"
echo "  Backend:  cd minimal-api-dev-v6 && npm run dev"
echo "  Frontend: cd vite-js && npm run dev"
echo ""
echo "Then visit: http://localhost:3031/dashboard/periodontics"



