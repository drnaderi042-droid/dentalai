"""
Heatmap-based 31-landmark detector - High accuracy for sub-2px MRE
Based on hrnet_p1p2_heatmap_best.pth approach but for 31 landmarks
"""
import torch
...
class HRNet31HeatmapDetector(nn.Module):
    """
    HRNet-based heatmap detector for 31 cephalometric landmarks.
    Outputs 31 heatmaps, then extracts coordinates from heatmaps.
    Target: < 2px MRE (similar to hrnet_p1p2_heatmap_best.pth)
    """
    def __init__(self, num_landmarks=31, hrnet_variant='hrnet_w32', pretrained=True, output_size=384):
        super(HRNet31HeatmapDetector, self).__init__()
        self.num_landmarks = num_landmarks
        self.output_size = output_size  # Higher resolution heatmap (384x384 for 768px input)

        if TIMM_AVAILABLE:
            # Load HRNet backbone - using larger variant for better accuracy
        ...
        # Enhanced heatmap decoder for 31 landmarks
        # Adaptive upsampling based on feature input size and target heatmap size
        # HRNet typically outputs features at 1/32 of input size, we need to upscale to heatmap_size

        # Calculate required upsampling factor
        # Assuming HRNet outputs ~24x24 features for 768x768 input, we need to reach heatmap_size
        ...
        self.heatmap_decoder = nn.Sequential(
            ...
            # Generate heatmaps
            nn.Conv2d(32, num_landmarks, 1),  # Output: (batch, num_landmarks, H, W)
                ...
                if heatmap_filtered.sum() > 0:
                    ...
                    if max_y > 0 and max_y < H-1 and max_x > 0 and max_x < W-1:
                        ...
                        try:
                            ...
                            y_coords = torch.arange(max_y-1, max_y+2, dtype=torch.float32)
                            x_coords = torch.arange(max_x-1, max_x+2, dtype=torch.float32)
