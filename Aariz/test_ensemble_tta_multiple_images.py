"""
Test Combined Ensemble + TTA on multiple images from validation set
This combines both ensemble (256x256 + 512x512) and Test-Time Augmentation
"""
...
def main():
    print("="*100)
    print("Test Combined Ensemble + TTA on Multiple Images")
    print("="*100)
    print("\nThis test combines:")
    print("  1. Ensemble of 256x256 + 512x512 models")
    ...
    for image_id in tqdm(image_ids, desc="Processing images"):
        ...
        try:
            ...
            # Ensemble with TTA (apply TTA to both models, then ensemble)
            pred_256_tta = predict_with_tta(predictor_256, img, target_size=(256, 256))
            pred_512_tta = predict_with_tta(predictor_512, img, target_size=(512, 512))
            ensemble_tta_pred = ensemble_predictions(pred_256_tta, pred_512_tta, LANDMARK_PREFERENCE)
            errors_ensemble_tta = calculate_errors(ensemble_tta_pred, gt_landmarks)
            metrics_ensemble_tta = calculate_metrics(errors_ensemble_tta)
            ...
            results_ensemble_tta.append({
                'image_id': image_id,
                'mre_mm': metrics_ensemble_tta['mre_mm'],
                'sdr_2mm': metrics_ensemble_tta['sdr_2mm']
            })
    ...
    mre_ensemble_tta = [r['mre_mm'] for r in results_ensemble_tta]
    sdr_ensemble_tta = [r['sdr_2mm'] for r in results_ensemble_tta]
    ...
    print(f"\n{'Metric':<30} {'256x256':<18} {'512x512':<18} {'Ensemble':<18} {'Ensemble+TTA':<18}")
    print("-"*102)
    print(f"{'Mean MRE (mm)':<30} {np.mean(mre_256):<18.4f} {np.mean(mre_512):<18.4f} {np.mean(mre_ensemble):<18.4f} {np.mean(mre_ensemble_tta):<18.4f}")
    print(f"{'Median MRE (mm)':<30} {np.median(mre_256):<18.4f} {np.median(mre_512):<18.4f} {np.median(mre_ensemble):<18.4f} {np.median(mre_ensemble_tta):<18.4f}")
    print(f"{'Mean SDR @ 2mm (%)':<30} {np.mean(sdr_256):<18.2f} {np.mean(sdr_512):<18.2f} {np.mean(sdr_ensemble):<18.2f} {np.mean(sdr_ensemble_tta):<18.2f}")
    print(f"{'Median SDR @ 2mm (%)':<30} {np.median(sdr_256):<18.2f} {np.median(sdr_512):<18.2f} {np.median(sdr_ensemble):<18.2f} {np.median(sdr_ensemble_tta):<18.2f}")
    
    # Improvement analysis
    best_single_sdr = max(np.mean(sdr_256), np.mean(sdr_512))
    ensemble_improvement = np.mean(sdr_ensemble) - best_single_sdr
    ensemble_tta_improvement = np.mean(sdr_ensemble_tta) - best_single_sdr
    tta_additional_improvement = np.mean(sdr_ensemble_tta) - np.mean(sdr_ensemble)
    
    print(f"\n" + "="*100)
    print("IMPROVEMENT ANALYSIS")
    print("="*100)
    print(f"Best single model SDR @ 2mm: {best_single_sdr:.2f}%")
    print(f"Ensemble SDR @ 2mm: {np.mean(sdr_ensemble):.2f}% (improvement: {ensemble_improvement:+.2f}%)")
    print(f"Ensemble+TTA SDR @ 2mm: {np.mean(sdr_ensemble_tta):.2f}% (improvement: {ensemble_tta_improvement:+.2f}%)")
    print(f"Additional improvement from TTA: {tta_additional_improvement:+.2f}%")
    
    best_mre = min(np.mean(mre_256), np.mean(mre_512))
    print(f"\nBest single model MRE: {best_mre:.4f} mm")
    print(f"Ensemble MRE: {np.mean(mre_ensemble):.4f} mm")
    print(f"Ensemble+TTA MRE: {np.mean(mre_ensemble_tta):.4f} mm")
    ...
    output = {
        ...
        'overall_stats': {
            ...
            'ensemble_tta': {
                'mean_mre': float(np.mean(mre_ensemble_tta)),
                'median_mre': float(np.median(mre_ensemble_tta)),
                'mean_sdr_2mm': float(np.mean(sdr_ensemble_tta)),
                'median_sdr_2mm': float(np.median(sdr_ensemble_tta))
            },
            'improvements': {
                'ensemble_vs_best_single_sdr': float(ensemble_improvement),
                'ensemble_tta_vs_best_single_sdr': float(ensemble_tta_improvement),
                'tta_additional_improvement': float(tta_additional_improvement)
            }
        },
        'per_image_results': [
            {
                ...
                'ensemble_tta': {'mre_mm': results_ensemble_tta[i]['mre_mm'], 'sdr_2mm': results_ensemble_tta[i]['sdr_2mm']}
            }
            for i in range(len(results_256))
        ]
    }
    
    output_file = 'test_ensemble_tta_multiple_images_results.json'
    ...
    print("CONCLUSION")
    print("="*100)
    if ensemble_tta_improvement > 0:
        print(f"Ensemble+TTA provides the best results!")
        print(f"  SDR @ 2mm: {np.mean(sdr_ensemble_tta):.2f}% (vs {best_single_sdr:.2f}% for best single model)")
        print(f"  Total improvement: {ensemble_tta_improvement:+.2f}%")
    print("\n" + "="*100)
