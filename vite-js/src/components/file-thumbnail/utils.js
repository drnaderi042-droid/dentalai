...
export function fileData(file) {
  ...
  // From file-like object (has name, size, path, etc.)
  // This handles both File objects and custom file-like objects
  return {
    name: file.name || fileNameByUrl(file.path || file.preview || ''),
    size: file.size,
    path: file.path || file.preview || '',
    type: file.type || fileTypeByUrl(file.path || file.preview || ''),
