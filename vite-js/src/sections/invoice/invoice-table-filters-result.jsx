...
export function InvoiceTableFiltersResult({ filters, totalResults, onResetPage, sx }) {
  ...
  return (
    <FiltersResult totalResults={totalResults} onReset={filters.onResetState} sx={sx}>
      <FiltersBlock label="خدمات:" isShow={!!filters.state.service.length}>
      ...
      <FiltersBlock label="وضعیت:" isShow={filters.state.status !== 'all'}>
        <Chip
          {...chipProps}
          label={
            filters.state.status === 'paid' ? 'پرداخت شده' :
            filters.state.status === 'pending' ? 'در انتظار' :
            filters.state.status === 'overdue' ? 'سررسید گذشته' :
            filters.state.status === 'draft' ? 'پیش‌نویس' : filters.state.status
          }
