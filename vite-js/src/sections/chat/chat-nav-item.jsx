...
export function ChatNavItem({ selected, collapse, conversation, onCloseMobile }) {
  ...
  const singleParticipant = (participants && participants[0]) || {};

  const { name = 'Unknown', avatarUrl = '', status = 'offline' } = singleParticipant;
  ...
  const renderSingle = (
    <Badge key={status} variant={status} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}>
      <Avatar alt={name} src={getAvatarUrl(avatarUrl)} sx={{ width: 48, height: 48 }} />
    </Badge>
  );
