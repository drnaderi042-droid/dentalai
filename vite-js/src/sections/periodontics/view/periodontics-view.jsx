...
export function PeriodonticsView() {
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        ...
        // Filter only Periodontics patients
        const periodonticsPatients = (response.data.patients || []).filter(
          (p) => p.specialty === 'PERIODONTICS'
        );
        
        // Fetch images for each patient in parallel
        const patientsWithImages = await Promise.all(
          periodonticsPatients.map(async (patient) => {
            try {
            ...
            return (
              <Grid item xs={12} sm={6} md={4} key={patient.id}>
                <Card
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => { navigate(`/dashboard/periodontics/patient/${patient.id}`); }}
                  onKeyDown={(e) => { if (e.key === 'Enter') { navigate(`/dashboard/periodontics/patient/${patient.id}`); } }}
                >
                  <Box sx={{ position: 'relative' }}>
                    ...
                    <Avatar
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/dashboard/periodontics/patient/${patient.id}`);
                      }}
                    />
