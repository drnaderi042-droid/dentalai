...
$TEETHDREAMER_DIR = Join-Path $SCRIPT_ROOT "TeethDreamer"
...
$VENV_ACTIVATE = Join-Path $SCRIPT_ROOT "venv_teethdreamer\Scripts\Activate.ps1"

# بررسی وجود محیط مجازی
if (-not (Test-Path $VENV_ACTIVATE)) {
    Write-Host "خطا: محیط مجازی پیدا نشد: $VENV_ACTIVATE" -ForegroundColor Red
    Write-Host "لطفا ابتدا محیط مجازی را ایجاد کنید." -ForegroundColor Yellow
    exit 1
}
...
# اجرای Generation
# توجه: test_set_npy لازم نیست (استفاده نمی‌شود) اما باید یک مقدار به آن بدهیم
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host ""
    Write-Host "لطفا خطاهای بالا را بررسی کنید." -ForegroundColor Yellow
...
# پیدا کردن فایل‌های تولید شده
Write-Host "جستجوی فایل‌های تولید شده..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "============================================" -ForegroundColor Cyan
Write-Host "مرحله 2: Reconstruction (بازسازی 3D)" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
...
Write-Host "دستور:" -ForegroundColor Yellow
Write-Host $reconstructionCmd -ForegroundColor Gray
Write-Host ""
...
# نمایش فایل‌های 3D نهایی
Write-Host "فایل‌های 3D تولید شده:" -ForegroundColor Yellow
$objFiles = Get-ChildItem -Path $RECONSTRUCTION_DIR -Recurse -Include *.ply,*.obj,*.stl
if ($objFiles.Count -gt 0) {
    $objFiles | ForEach-Object {
        Write-Host "  - $($_.FullName) ($([math]::Round($_.Length / 1MB, 2)) MB)" -ForegroundColor Green
    }
} else {
    Write-Host "  (هنوز فایل 3D تولید نشده است - ممکن است در دایرکتوری‌های زیرپوشه باشد)" -ForegroundColor Gray
    Get-ChildItem -Path $RECONSTRUCTION_DIR -Recurse -Directory | ForEach-Object {
