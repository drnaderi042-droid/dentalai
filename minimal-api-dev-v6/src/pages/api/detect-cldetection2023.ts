...
/**
 * Proxy endpoint for CLdetection2023 Cephalometric Detection
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // 🔧 FIX: Use unified_ai_api_server (port 5001) for CLdetection2023
    const serviceUrl = process.env.AI_SERVER_URL || 'http://localhost:5001';

    console.log('[Detect CLdetection2023] Proxying request to:', `${serviceUrl}/detect-cldetection2023`);
    console.log('[Detect CLdetection2023] Has image_url:', !!image_url);
    console.log('[Detect CLdetection2023] Has image_base64:', !!image_base64);
    ...
    try {
      ...
      const response = await fetch(`${serviceUrl}/detect-cldetection2023`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        console.error('[Detect CLdetection2023] Error response:', errorText);
      ...
      console.log('[Detect CLdetection2023] Success! Response:', {
      ...
      console.error('[Detect CLdetection2023] Detection failed:', error.message);
    ...
    console.error('[Detect CLdetection2023] Error:', error);
    return res.status(500).json({
      success: false,
      error: error.message || 'Internal server error',
    });
